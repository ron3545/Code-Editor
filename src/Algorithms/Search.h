...
public:
    ...
    {
        std::string m_Line;
        size_t m_offset, m_lineNumber;
        Handler_KeyLocation(const std::string& Line, size_t offset, size_t lineNumber)
    ...
    struct Handler_SearchKeyOnFile
    {
        ...
        Handler_SearchKeyOnFile(size_t occurrences, const std::string& key,
...
private:    
    ...
    void SearchOnLine(const Line&, std::string_view key, size_t line_number, size_t* occurances, KeyLocations* lines);
};
