...
bool ImGui::TreeNodeWithImageBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, void* textureID, bool isDirectory, bool static_highlight)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Render
    if(textureID != nullptr && !isDirectory)  
        window->DrawList->AddImage( textureID, ImVec2(text_pos.x - 17, text_pos.y + 4), 
                                    ImVec2(text_pos.x - 6, text_pos.y + 17));
    ...
    else
    {
        ...
        // Unframed typed for tree nodes
        if (hovered || selected || static_highlight)
        {
            const ImU32 bg_col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
            RenderFrame(frame_bb.Min, frame_bb.Max, bg_col, false);
        }
    ...
    return (isDirectory)? is_open : pressed;
}
